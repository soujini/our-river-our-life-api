...
passport.use('id', new BasicStrategy((id, done) => {
  const userSchema = new Schema({ id: schema.tree.id})

  userSchema.validate({ id }, (err) => {
    if (err) done(err)
  })
  ...
  User.findOne({ id }).then((user) => {
    ...
    return user.authenticate(id, user.id).then((user) => {
      done(null, user)
      return null
    }).catch(done)
  })
